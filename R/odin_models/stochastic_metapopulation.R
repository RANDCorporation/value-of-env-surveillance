...
dim(L_star_ind) <- n # Jurisdiction's own L_star without considering others
dim(L_star_max) <- c(n, n) # Target NPI considering maximum NPI level of coordinating jurisdictions.
dim(lagged_incidence) <- n # Lagged incidence vector
...
# This implementation replaces the delay function, which I found to be unreliable:
...
# Update Non-pharmaceutical target intervention level with an exponentially-weighted moving average
