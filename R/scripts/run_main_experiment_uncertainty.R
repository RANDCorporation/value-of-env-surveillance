...
# Visualize posterior:
# Set posterior
model$set_param_dist(params_list = list(a = posterior),
...
comp_results <- base_results %>%
  ...
  select(rep, param.id, counterfactual.id, any_of(r$outcomes)) %>%
  clear.labels() %>%
...
base_res_diff <- base_results %>%
  clear.labels() %>%
  left_join(comp_results, by = join_by(rep, param.id, counterfactual.id))
...
base_results_rep_summaries <- base_res_diff %>%
  select(-c(rep, grid.id, lhs.id, params_design.id, param_dist.df.name, param_dist.df.id, param_dist.df.row.id, param_dist.orig.row.id, param.id, model.id, all.params.id, c, c_mult,R0_mult,tau_set,fixed_tau, p, total_surv_lag, a_up, a_down, p, R0, r, cost_max_npi, tau, policy.exp.id)) %>%
