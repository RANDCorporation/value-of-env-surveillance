...
Policy stringency (i.e., how stringent the case thresholds used to introduce or remove interventions) has a similar effect on the value of EWS relative to decision lags - the EWS provides the most value under the conditions that are least desirable for prevention of disease. In a context where decision makers are 50% more stringent than in our base-case assumptions (i.e., their case tresholds are half of its base-case), society is able to avert a substantial number of deaths, reducing deaths from `r pull_estimate(r$table_1_long_all, Scenario == "2-week decision lag", variable == "deaths_per_100k", NMB_comparator == T)`
...
In our base-line assumptions, we assumed decision-makers take one week to update their risk classification levels based on the epidemiological data. That decision lag includes both the time it takes for public health decision-makers to 
